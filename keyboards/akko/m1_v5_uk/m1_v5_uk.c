        ...
        case RGB_VAD: {
            ...
            if (record->event.pressed) {
                if (rgb_matrix_get_val() <= RGB_MATRIX_VAL_STEP) {
                    ...
                    if (!(host_keyboard_led_state().caps_lock || (!keymap_is_mac_system() && keymap_config.no_gui))){
...
void bat_indicators(void) {
    ...
    if (charging_state && (bat_full_flag)) {
        ...
        battery_process_time = 0;
        if (!led_status && rgb_matrix_get_val()) rgb_matrix_set_color(HS_MATRIX_BLINK_INDEX_BAT, 0x00, 0xFF, 0x00);
