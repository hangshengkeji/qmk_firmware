...
bool process_record_kb(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case KC_1: { //
            ...
            if (confinfo.hs_ct) {
                if (record->event.pressed) {
                    if (!confinfo.mult_filp) register_code16(KC_F1); // F1
                    else register_code16(KC_BRID);
                } else {
                    if (!confinfo.mult_filp) unregister_code16(KC_F1); // F1
                    else unregister_code16(KC_BRID);
                }
                return false;
            }
        } break;
        ...
        case KC_2: { //
            ...
            if (confinfo.hs_ct) {
                if (record->event.pressed) {
                    if (!confinfo.mult_filp) register_code16(KC_F2); // F1
                    else register_code16(KC_BRIU);
                } else {
                    if (!confinfo.mult_filp) unregister_code16(KC_F2); // F1
                    else unregister_code16(KC_BRIU);
                }
                return false;
            }
        } break;
        ...
        case KC_3: { //
            ...
            if (confinfo.hs_ct) {
                if (record->event.pressed) {
                    if (!confinfo.mult_filp) register_code16(KC_F3); // F1
                    else {
                    ...
                    if (!confinfo.mult_filp) unregister_code16(KC_F3); // F1
                    else {
                        if (keymap_is_mac_system()) {
                            unregister_code16(KC_LGUI);
                            unregister_code16(KC_TAB);
        ...
        case KC_4: {
            if (confinfo.hs_ct) {
                if (record->event.pressed) {
                    if (!confinfo.mult_filp) register_code16(KC_F4); // f6
                    else register_code16(KC_WSCH);
                } else {
                    if (!confinfo.mult_filp) unregister_code16(KC_F4); // f6
                    else unregister_code16(KC_WSCH);
                }
                return false;
            }
        } break;
        case KC_5: {
            if (confinfo.hs_ct) {
                if (record->event.pressed) {
                    if (!confinfo.mult_filp) register_code16(KC_F5); // F1
                    else {
                        if (get_backlight_level() == 0) return false;
                        if ((confinfo.backlight_mode != 1) && (confinfo.backlight_mode != 2)) return false;
                        backlight_decrease();
                    ...
                    if (!confinfo.mult_filp) unregister_code16(KC_F5); // F1
                    else {
        ...
        case KC_6: {
            if (confinfo.hs_ct) {
                if (record->event.pressed) {
                    if (!confinfo.mult_filp) register_code16(KC_F6); // F5
                    else {
                        if (get_backlight_level() == 4) return false;
                        if ((confinfo.backlight_mode != 1) && (confinfo.backlight_mode != 2)) return false;
                        backlight_increase();
                    ...
                    if (!confinfo.mult_filp) unregister_code16(KC_F6); // F5
                    else {
        ...
        case KC_7: {
            if (confinfo.hs_ct) {
                if (record->event.pressed) {
                    if (!confinfo.mult_filp) register_code16(KC_F7); // f7
                    else register_code16(KC_MPRV);
                } else {
                    if (!confinfo.mult_filp) unregister_code16(KC_F7); // f7
                    else unregister_code16(KC_MPRV);
                }
                return false;
            }
        ...
        case KC_8: {
            if (confinfo.hs_ct) {
                if (record->event.pressed) {
                    if (!confinfo.mult_filp) register_code16(KC_F8); // f8
                    else register_code16(KC_MPLY);
                } else {
                    if (!confinfo.mult_filp) unregister_code16(KC_F8); // f8
                    else unregister_code16(KC_MPLY);
                }
                return false;
            }
        ...
        case KC_9: {
            if (confinfo.hs_ct) {
                if (record->event.pressed) {
                    if (!confinfo.mult_filp) register_code16(KC_F9); // f9
                    else register_code16(KC_MNXT);
                } else {
                    if (!confinfo.mult_filp) unregister_code16(KC_F9); // f9
                    else unregister_code16(KC_MNXT);
                }
                return false;
            }
        ...
        case KC_0: {
            if (confinfo.hs_ct) {
                if (record->event.pressed) {
                    if (!confinfo.mult_filp) register_code16(KC_F10); // f10
                    else register_code16(KC_MUTE);
                } else {
                    if (!confinfo.mult_filp) unregister_code16(KC_F10); // f10
                    else unregister_code16(KC_MUTE);
                }
                return false;
            }
        ...
        case KC_MINS: {
            if (confinfo.hs_ct) {
                if (record->event.pressed) {
                    if (!confinfo.mult_filp) register_code16(KC_F11); // f11
                    else register_code16(KC_VOLD);
                } else {
                    if (!confinfo.mult_filp) unregister_code16(KC_F11); // f11
                    else unregister_code16(KC_VOLD);
                }
                return false;
            }
        ...
        case KC_EQL: {
            if (confinfo.hs_ct) {
                if (record->event.pressed) {
                    if (!confinfo.mult_filp) register_code16(KC_F12); // F12
                    else register_code16(KC_VOLU);
                } else {
                    if (!confinfo.mult_filp) unregister_code16(KC_F12); // F12
                    else unregister_code16(KC_VOLU);
                }
                return false;
            }
