...
// const uint8_t sixth_gear_buff[]  = {0};
const uint8_t rgb_hsvs[RGB_HSV_MAX][2] = {
    {0,255},  //红
    {43,255}, //黄
    {85,255}, //绿
    {106,255}, //青
...
void eeconfig_init_user_datablock(void) {
    ...
    addr = ((uint32_t)CONFINFO_EECONFIG_ADDR + 4) + 16;
...
uint8_t record_color_read_sat(void) {
    uint8_t hs_mode    = find_index();
    const uint8_t *ptr = (const uint8_t *)(((uint32_t)CONFINFO_EECONFIG_ADDR + 4) + 16 + hs_mode);
    uint8_t hs_c       = eeprom_read_byte(ptr);
    return hs_c;
...
void record_rgbmatrix_increase(uint8_t *last_mode) {
    ...
    *last_mode = rgbmatrix_buff[index];
    ...
    rgb_matrix_sethsv(rgb_hsvs[rgb_hsv_index][0], record_color_read_sat(), rgb_matrix_get_val());
}
...
uint8_t record_color_hsv(bool status) {
    ...
    // if (status){
    //     if (rgb_hsv_index < 5) rgb_hsv_index += 1;
    // } else {
    //     if (rgb_hsv_index > 0) rgb_hsv_index -= 1;
    // }
...
uint8_t record_color_sat(bool status) {

    if (status) rgb_matrix_increase_sat();
    else rgb_matrix_decrease_sat();


    uint8_t *ptr = (uint8_t *)(((uint32_t)CONFINFO_EECONFIG_ADDR + 4) + 16 + find_index());
    eeprom_write_byte(ptr, rgb_matrix_get_sat());
    return 0;
}
...
void query(void) {
    extern uint8_t inqbat_flag;
    if (inqbat_flag) {
// #ifdef RGBLIGHT_ENABLE
//         for (uint8_t i = 0; i < (RGB_MATRIX_LED_COUNT - RGBLIGHT_LED_COUNT); i++) {
