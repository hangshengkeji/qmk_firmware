...
void palcallback_hook(uint8_t line) __attribute__((weak));
...
void palcallback(void *arg) {
    ...
    switch (line) {
        ...
        case PAL_PAD(UART_RX_PIN): {
            lpwr_set_wakeupcd(LPWR_WAKEUP_UART);
        } break;
#endif
        default: {
...
void lpwr_exti_init_hook(lpwr_mode_t mode) __attribute__((weak));
void lpwr_exti_init_hook(lpwr_mode_t mode) {}

void lpwr_exti_init(lpwr_mode_t mode) {
...
#ifndef LPWR_UART_WAKEUP_DISABLE
    setPinInputHigh(UART_RX_PIN); // 此处修改为上拉可能会影响功耗 setPinInput(UART_RX_PIN);
    waitInputPinDelay();
    palEnableLineEvent(UART_RX_PIN, PAL_EVENT_MODE_FALLING_EDGE);
#endif

    lpwr_exti_init_hook(mode);
}
...
#ifndef LPWR_UART_WAKEUP_DISABLE
    palSetLineMode(UART_RX_PIN, PAL_MODE_ALTERNATE(UART_RX_PAL_MODE) | PAL_OUTPUT_TYPE_PUSHPULL | PAL_WB32_PUPDR_PULLUP | PAL_OUTPUT_SPEED_HIGHEST);
#endif
...
void wb32_stop_mode(void) {
    ...
    CACHE->CR = CACHE_CR_CHEEN | CACHE_CR_PREFEN_ON | CACHE_CR_LATENCY_4WS;
