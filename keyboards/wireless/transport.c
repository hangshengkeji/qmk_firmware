...
#else
    static uint32_t suspend_entry_timer        = 0x00;
    static uint32_t last_matrix_activity_timer = 0x00;

    if ((USB_DRIVER.state == USB_SUSPENDED)) {
        if (!suspend_timer) suspend_timer = sync_timer_read32();
        if (!suspend_entry_timer) suspend_entry_timer = sync_timer_read32();
        if (last_matrix_activity_timer != last_matrix_activity_time()) {
            last_matrix_activity_timer = last_matrix_activity_time();
            suspend_entry_timer        = sync_timer_read32();
        }
        if (sync_timer_elapsed32(suspend_entry_timer) >= USB_POWER_DOWN_DELAY) {
            suspend_entry_timer = 0x00;
#    ifdef USB_CABLE_PIN
...
bool pre_process_record_kb(uint16_t keycode, keyrecord_t *record) {
    ...
    if (pre_process_record_user(keycode, record) != true) {
        return false;
    }
...
#    endif
        {
            if (get_usb_suspend_state()) usb_remote_host();
        }
    }
...
bool get_usb_suspend_state(void) {
    ...
    return suspend_timer && (sync_timer_elapsed32(suspend_timer) >= USB_POWER_DOWN_DELAY);
}
#endif
