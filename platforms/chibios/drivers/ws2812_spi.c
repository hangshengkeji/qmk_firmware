...
#    ifdef HS_MUSIC_LED_COUNT
    for (int i = 0; i < WS2812_LED_COUNT - HS_MUSIC_LED_COUNT; i++) {
        set_led_color_rgb(ws2812_leds[i], i);
    }
    extern uint8_t ee_clr_flag;
    if (ee_clr_flag) {
        for (int i = 0; i < HS_MUSIC_LED_COUNT; i++) {
            set_led_color_rgb(ws2812_leds[WS2812_LED_COUNT - HS_MUSIC_LED_COUNT + i], WS2812_LED_COUNT - HS_MUSIC_LED_COUNT + i);
        }
    }
#    else
    for (int i = 0; i < WS2812_LED_COUNT; i++) {
